<div>
    ...
    <div class="space-y-6">
        ...
        @if($budgets->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($budgets as $budget)
                    <div class="bg-white dark:bg-gray-800 rounded-xl border
                        @if($budget->status === 'near-limit') border-yellow-300 dark:border-yellow-600
                        @elseif($budget->status === 'over-budget') border-red-300 dark:border-red-600
                    ...
                    >
                        ...
                        {{-- Category Name --}}
                        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{{ $budget->category->name }}</h3>

                        {{-- Amounts --}}
                        <div class="mb-4">
                            <div class="flex justify-between items-baseline gap-2">
                                <span class="text-3xl font-bold text-gray-900 dark:text-gray-100">₦{{ number_format($budget->spent, 0) }}</span>
                                <span class="text-gray-500 dark:text-gray-400">of ₦{{ number_format($budget->amount, 0) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-300
                                    @if($budget->status === 'over-budget') bg-red-500 dark:bg-red-600
                                    @elseif($budget->status === 'near-limit') bg-yellow-500 dark:bg-yellow-600
                                    @else bg-gradient-to-r from-indigo-500 to-purple-500 dark:from-indigo-600 dark:to-purple-600
                                    @endif"
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="font-semibold
                                @if($budget->status === 'over-budget') text-red-600 dark:text-red-400
                                @elseif($budget->status === 'near-limit') text-yellow-600 dark:text-yellow-400
                                @else text-green-600 dark:text-green-400
                                @endif">
                                {{ number_format($budget->percentage, 1) }}% used
                            </span>
                            <span class="text-gray-500 dark:text-gray-400">₦{{ number_format($budget->remaining, 0) }} left</span>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            {{-- Budget Summary --}}
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-6">Budget Summary</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Total Allocated</p>
                        <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">₦{{ number_format($totalAllocated, 0) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Total Spent</p>
                        <p class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">₦{{ number_format($totalSpent, 0) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Remaining</p>
                        <p class="text-3xl font-bold text-green-600 dark:text-green-400">₦{{ number_format($totalRemaining, 0) }}</p>
                    </div>
                </div>
            </div>
        @else
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12">
                <div class="text-center text-gray-400 dark:text-gray-500">
                    ...
                    <p class="text-lg font-medium text-gray-600 dark:text-gray-300 mb-2">No budgets set for {{ $months[$selectedMonth] }} {{ $selectedYear }}</p>
                    <p class="text-sm mb-4">Create your first budget to start tracking your spending</p>
                    <x-primary-button x-on:click="$dispatch('open-modal', 'add-budget')"
                            class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600 text-white font-medium rounded-lg transition">
